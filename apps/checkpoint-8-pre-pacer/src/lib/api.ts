...
// --- Infinite, server-sorted, paginated flights (Book a Flight table) ---

const FLIGHTS_PAGE_SIZE = 50
...
async function fetchFlightsPage(
  ...
  const params = new URLSearchParams({
    status: "scheduled",
    _page: String(pageParam + 1), // json-server pages are 1-indexed
...
/**
 * Shared options for the Book tab's infinite table. Living here (rather than
 * inline in the route) means the dashboard tab bar can prefetch the exact same
 * query on hover — the key is built from the same values, so it always matches.
 */
export function flightsInfiniteQuery(
  sorting: SortSpec[],
  filters: FlightFilters
) {
