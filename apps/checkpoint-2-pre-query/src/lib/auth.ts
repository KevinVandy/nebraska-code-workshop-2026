...
/**
 * Read the session from the browser's cookie. This is what route
 * `beforeLoad`s call — the cookie transport and the guard are REAL; only the
 * cookie's contents being unsigned is fake (see the header comment).
 *
 * This app is a client-only SPA, so there's only one place a session can come
 * from: document.cookie.
 */
export function readSession(): Session | null {
  return sessionFromCookieHeader(document.cookie)
}
