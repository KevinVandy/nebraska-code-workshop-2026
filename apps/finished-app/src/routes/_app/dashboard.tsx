...
export const Route = createFileRoute("/_app/dashboard")({
  /* `data-only` runs loaders/beforeLoad on the server but ships no rendered
   * HTML for this subtree. The dashboard is behind auth and personal to each
   * user, so server-rendering its markup buys nothing — but running the guard
   * server-side still means a signed-out visitor is redirected before any
   * dashboard JS is downloaded. */
  ssr: "data-only",
      ...
      [
        ...
        },
        {
          ...
          prefetch: () => {
            ...
            // tab opens in, so the key matches what useInfiniteQuery will ask
            // for.
